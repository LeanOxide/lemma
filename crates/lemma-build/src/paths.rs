...
impl BuildPaths {
    ...
    /// Get the .olean.server path for a module
    ///
    /// Language server artifacts are automatically generated by Lean when isModule=true.
    /// Example: "Foo.Bar" -> ".lake/build/lib/lean/Foo/Bar.olean.server"
    pub fn olean_server_path(&self, module_name: &str) -> PathBuf {
        let olean = self.olean_path(module_name);
        olean.with_extension("olean.server")
    }

    /// Get the .olean.private path for a module
    ///
    /// Language server artifacts are automatically generated by Lean when isModule=true.
    /// Example: "Foo.Bar" -> ".lake/build/lib/lean/Foo/Bar.olean.private"
    pub fn olean_private_path(&self, module_name: &str) -> PathBuf {
        let olean = self.olean_path(module_name);
        olean.with_extension("olean.private")
    }
    ...
    /// Get the setup JSON file path for a module
    ///
    /// The setup file contains module metadata passed to the Lean compiler via --setup.
    /// Example: "Foo.Bar" -> ".lake/build/ir/Foo/Bar.setup.json"
    pub fn setup_path(&self, module_name: &str) -> PathBuf {
        let parts: Vec<&str> = module_name.split('.').collect();
        let mut path = self.ir_dir();
        for part in parts {
            path.push(part);
        }
        path.set_extension("setup.json");
        path
    }
...
mod tests {
    ...
    #[test]
    fn test_language_server_artifact_paths() {
        let temp_dir = TempDir::new().unwrap();
        let project_dir = temp_dir.path().to_path_buf();
        let build_dir = project_dir.join(".lake/build");
        ...
        assert_eq!(
            paths.olean_server_path("Foo.Bar"),
            build_dir.join("lib/lean/Foo/Bar.olean.server")
        );

        assert_eq!(
            paths.olean_private_path("Foo.Bar"),
            build_dir.join("lib/lean/Foo/Bar.olean.private")
        );
    }
}
